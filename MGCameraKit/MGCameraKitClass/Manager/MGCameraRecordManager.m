...
@interface MGCameraRecordManager ()
{
...
@implementation MGCameraRecordManager
...
- (void)dealloc {
    ...
    if ( _videoTrackSourceFormatDescription ) {
        //        _videoTrackSourceFormatDescription = nil;
...
- (void)addVideoTrackWithSourceFormatDescription:(CMFormatDescriptionRef)formatDescription transform:(CGAffineTransform)transform {
    ...
    @synchronized( self ) {
        if ( _status != MGCameraRecordStatusIdle ) {
        ...
        //        _videoTrackSourceFormatDescription = (CMFormatDescriptionRef)formatDescription;
...
- (id<MGCameraRecordDelegate>)delegate {
    id <MGCameraRecordDelegate> delegate = nil;
    @synchronized( self ) {
...
- (void)prepareToRecord {
    @synchronized( self ) {
        ...
        [self transitionToStatus:MGCameraRecordStatusPreparingToRecord error:nil];
    }
    ...
    dispatch_async( dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0 ), ^{
        @autoreleasepool {
            ...
            @synchronized( self ) {
                if ( error )
                    [self transitionToStatus:MGCameraRecordStatusFailed error:error];
                else
                    [self transitionToStatus:MGCameraRecordStatusRecording error:nil];
            }
        }
    });
}
...
- (void)finishRecording {
    @synchronized( self ) {
        ...
        switch ( _status ) {
            case MGCameraRecordStatusIdle:
            case MGCameraRecordStatusPreparingToRecord:
            case MGCameraRecordStatusFinishingRecordingPart1:
            case MGCameraRecordStatusFinishingRecordingPart2:
            case MGCameraRecordStatusFinished:
                @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Not recording" userInfo:nil];
                break;
            case MGCameraRecordStatusFailed:
                // From the client's perspective the movie recorder can asynchronously transition to an error state as the result of an append.
                // Because of this we are lenient when finishRecording is called and we are in an error state.
                NSLog( @"Recording has failed, nothing to do" );
                break;
            case MGCameraRecordStatusRecording:
                shouldFinishRecording = YES;
                break;
        }
        
        if ( shouldFinishRecording )
            [self transitionToStatus:MGCameraRecordStatusFinishingRecordingPart1 error:nil];
        else
            return;
    }
    ...
    dispatch_async( _writingQueue, ^{
        @autoreleasepool {
            @synchronized( self ) {
                ...
                if ( _status != MGCameraRecordStatusFinishingRecordingPart1 )
                    return ;
                
                // It is not safe to call -[AVAssetWriter finishWriting*] concurrently with -[AVAssetWriterInput appendSampleBuffer:]
                // We transition to MGCameraRecordStatusFinishingRecordingPart2 while on _writingQueue, which guarantees that no more buffers will be appended.
                [self transitionToStatus:MGCameraRecordStatusFinishingRecordingPart2 error:nil];
            }
            ...
            dispatch_block_t completionHandler = ^{
                @synchronized( self ) {
                    ...
                    if ( error )
                        ...
                        [self transitionToStatus:MGCameraRecordStatusFinished error:nil];
                }
            };
...
- (void)appendSampleBuffer:(CMSampleBufferRef)sampleBuffer ofMediaType:(NSString *)mediaType {
    ...
    @synchronized( self ) {
        if ( _status < MGCameraRecordStatusRecording ) {
...
- (void)transitionToStatus:(MGCameraRecordStatus)newStatus error:(NSError *)error {
    ...
    if ( newStatus != _status ) {
        // terminal states
        if ( ( newStatus == MGCameraRecordStatusFinished ) || ( newStatus == MGCameraRecordStatusFailed ) ) {
            shouldNotifyDelegate = YES;
            // make sure there are no more sample buffers in flight before we tear down the asset writer and inputs
            
            dispatch_async( _writingQueue, ^{
                [self teardownAssetWriterAndInputs];
                if ( newStatus == MGCameraRecordStatusFailed ) {
                    [[NSFileManager defaultManager] removeItemAtURL:_URL error:NULL];
                }
            });
        ...
        else if ( newStatus == MGCameraRecordStatusRecording ) {
            shouldNotifyDelegate = YES;
        }
    ...
    if ( shouldNotifyDelegate && [self delegate] ) {
        dispatch_async( _delegateCallbackQueue, ^{
            @autoreleasepool {
                switch ( newStatus ) {
                    case MGCameraRecordStatusRecording:
                        if (_delegate && [_delegate respondsToSelector:@selector(mgCameraRecordDidFinishPreparing:)]) {
                            [_delegate mgCameraRecordDidFinishPreparing:self];
                        }
                        break;
                    case MGCameraRecordStatusFinished:
                        if (_delegate && [_delegate respondsToSelector:@selector(mgCameraRecordDidFinishPreparing:)]) {
                        ...
                        [[self delegate] mgCameraRecordDidFinishRecording:self];
                        break;
                    case MGCameraRecordStatusFailed:
                        if (_delegate && [_delegate respondsToSelector:@selector(mgCameraRecordDidFinishPreparing:)]) {
                            [_delegate mgCameraRecordDidFinishPreparing:self];
...
- (NSString *)stringForStatus:(MGCameraRecordStatus)status {
