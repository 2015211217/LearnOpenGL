/*
 * $File: linalg.h
 * $Date: Sat Sep 12 22:07:16 2015 +0800
 * $Author: Xinyu Zhou <zxytim[at]gmail[dot]com>
 */

/*!
 * Some primitive linear algebra tools
 * FIXME: Code below is copied from a previous project.
 *		Its implementation is very naive and not efficient.
 */
...
namespace linalg {
	/*!
	 * solve linear system Ax=b. note that @A and @b will be modified. result x is store in @b
	 */
...
#define AT(i, j) A[(i) * n + (j)]
			...
			for (uint32_t i = 0; i < n; i ++) {
				...
				for (uint32_t j = i + 1; j < n; j ++) {
					...
					switch (left) {
						case 7: row_j[k + 6] -= row_i[k + 6] * factor;
						case 6: row_j[k + 5] -= row_i[k + 5] * factor;
						case 5: row_j[k + 4] -= row_i[k + 4] * factor;
						case 4: row_j[k + 3] -= row_i[k + 3] * factor;
						case 3: row_j[k + 2] -= row_i[k + 2] * factor;
						case 2: row_j[k + 1] -= row_i[k + 1] * factor;
						case 1: row_j[k] -= row_i[k] * factor;
						case 0: ;
					}
...
#define AT(A, i, j) A[(i) * n + (j)]
			...
			auto mult_row_scalar = [&](value_type *A, uint32_t row, value_type f, uint32_t start) {
				for (size_t j = start; j < n; j ++)
					AT(A, row, j) *= f;
			};
...
#undef AT
	 ...
	 */
	template <class value_type>
		void mat_mult_non_square(const value_type *A, const value_type *B, value_type *C,
				uint8_t dim0, uint32_t dim1, uint32_t dim2) {
	 ...
	 * @param b  rows x 1
	 * @param x  cols x 1
	 * @param buf buffer used by pseudo_inverse_mat. see doc for pseudo_inverse_mat for detail.
	 */
	template <class value_type>
		void solve_pseudo(value_type *A, uint32_t rows, uint32_t cols, const value_type *b,
				value_type *x, value_type *buf) {
